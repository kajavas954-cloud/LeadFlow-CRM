...
export class AuthService {
  async register(name: string, email: string, password: string, role: Role = Role.SALES_MEMBER): Promise<Omit<User, 'password' | 'refreshToken'>> {
  ...
  async login(email: string, password: string): Promise<{ accessToken: string; refreshToken: string; user: Omit<User, 'password' | 'refreshToken'> }> {
  ...
  async refresh(token: string): Promise<{ accessToken: string; user: Omit<User, 'password' | 'refreshToken'> }> {
    try {
  ...
  async getUsers(): Promise<Omit<User, 'password' | 'refreshToken'>[]> {
