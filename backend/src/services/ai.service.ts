...
export class AIService {
  // 1. AI Lead Summary
  async generateLeadSummary(leadId: string): Promise<string> {
    ...
    const notesSummary = lead.notes.map((n: any) => n.note).join(' ');
    
    let plan = 'pricing details';
    if (notesSummary.toLowerCase().includes('enterprise')) plan = 'the Enterprise plan';
    if (notesSummary.toLowerCase().includes('proposal')) plan = 'a custom pricing proposal';

    let timeline = 'a follow-up soon';
    if (notesSummary.toLowerCase().includes('tuesday')) timeline = 'a follow-up call next Tuesday';
    if (notesSummary.toLowerCase().includes('tomorrow')) timeline = 'a touchpoint tomorrow';
    if (notesSummary.toLowerCase().includes('week')) timeline = 'a demonstration next week';
  ...
  async generateNextAction(leadId: string): Promise<{ action: string; description: string }> {
    const lead = await prisma.lead.findUnique({
      where: { id: leadId },
      include: { notes: true }
    });
  ...
  generateEmail(leadName: string, company: string, templateName: string): string {
  ...
  parseMeetingNotes(rawText: string): { summary: string; actionItems: string[]; nextFollowUp: string; decisions: string[] } {
    ...
    if (text.includes('proposal') || text.includes('send')) actionItems.push('Draft and send enterprise pricing proposal');
    if (text.includes('demo') || text.includes('schedule')) actionItems.push('Schedule technical platform demonstration');
    if (text.includes('call') || text.includes('phone')) actionItems.push('Call representative to clarify requirements');
    ...
    if (text.includes('tuesday')) nextFollowUp = 'Tuesday at 2:00 PM';
    if (text.includes('tomorrow')) nextFollowUp = 'Tomorrow morning';
    if (text.includes('next week')) nextFollowUp = 'Next week Monday';

    // Extrapolate decisions
    const decisions: string[] = ['Establish pipeline priorities.'];
    if (text.includes('enterprise')) decisions.push('Agreed to evaluate Enterprise subscription tier.');
    if (text.includes('agree') || text.includes('yes')) decisions.push('Agreed to schedule subsequent follow-up call.');
  ...
  parseNaturalSearch(prompt: string): any {
    const text = prompt.toLowerCase();
    const filters: any = {};

    // Sources
    if (text.includes('linkedin')) filters.source = 'LINKEDIN';
    if (text.includes('website')) filters.source = 'WEBSITE';
    if (text.includes('referral')) filters.source = 'REFERRAL';
    if (text.includes('cold email')) filters.source = 'COLD_EMAIL';
    if (text.includes('google ads')) filters.source = 'GOOGLE_ADS';

    // Priorities
    if (text.includes('urgent')) filters.priority = 'URGENT';
    if (text.includes('high-priority') || text.includes('high priority') || text.includes('high')) filters.priority = 'HIGH';
    if (text.includes('medium')) filters.priority = 'MEDIUM';
    if (text.includes('low')) filters.priority = 'LOW';

    // Statuses
    if (text.includes('new')) filters.status = 'NEW';
    if (text.includes('contacted')) filters.status = 'CONTACTED';
    if (text.includes('qualified')) filters.status = 'QUALIFIED';
    if (text.includes('proposal')) filters.status = 'PROPOSAL_SENT';
    if (text.includes('negotiation')) filters.status = 'NEGOTIATION';
    if (text.includes('won') || text.includes('closed won')) filters.status = 'WON';
    if (text.includes('lost') || text.includes('closed lost')) filters.status = 'LOST';
