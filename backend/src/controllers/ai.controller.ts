...
export class AIController {
  ...
  // 2. Next Best Action
  async getNextAction(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { leadId } = req.body;
      if (!leadId) {
        res.status(400).json({ success: false, message: 'leadId is required' });
  ...
  async parseNotes(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
  ...
  // 5. Natural Search
  async naturalSearch(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
